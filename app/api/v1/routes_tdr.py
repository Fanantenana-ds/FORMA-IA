...
async def generer_tdr(request: TDRRequest):
    ...
    resultat = await orchestrator.generate(brief)

    if resultat.get("status") == "error":
        logger.error(f"❌ Erreur TDR: {resultat.get('error')}")
        return TDRResponse(success=False, data={}, error=resultat.get("error"))

    logger.info(f"✅ TDR généré avec succès pour: {request.client}")
    return TDRResponse(success=True, data=resultat, error=None)
