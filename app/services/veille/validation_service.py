...
# ✅ FANITSIA 4 : Esory ny lohateny sasany mba tsy ho very ny vokatra
GENERIC_AGGREGATOR_TITLES = [
    "offres d'emploi gratuit",
    # "offres d'emploi digital et tech",  # Esory satria mety ho lohateny tena misy
    # "emplois développeur",  # Esory koa
    "jobs en madagascar", "résultats de recherche",
    "recherche d'emploi", "search jobs", "job search",
]

# ✅ FANITSIA 2 : Ampitomboy ny ACTION_WORDS
ACTION_WORDS = [
    ...
    # ✅ FANITSIA 2 : Ampitomboy ireo teny vaovao
...
def quality_filter(opportunity: Dict[str, Any]) -> bool:
    ...
    # ✅ FANITSIA 4 : Atao malefaka ny fepetra amin'ny agregateur
    is_aggregator = any(pattern in title_lower for pattern in GENERIC_AGGREGATOR_TITLES)
    if is_aggregator:
        if "emploi" in title_lower or "offre" in title_lower or "stage" in title_lower:
            logger.info("ℹ️ Agrégateur mais accepté car offre spécifique : %s", title)
        else:
            logger.info("🚫 Rejet agrégateur : %s", title)
            return False

    # ✅ FANITSIA 1 : Raha misy "data", "bureautique", "devops" dia avela handalo
    has_action = any(
    ...
    has_other_keyword = "bureautique" in title_lower or "devops" in title_lower or "freelance" in title_lower

    if not has_action and not has_data_keyword and not has_other_keyword:
        logger.info("🚫 Aucun indice d'action concrète : %s", title)
        return False
    elif not has_action and (has_data_keyword or has_other_keyword):
        logger.info("ℹ️ Action word faible mais accepté (data/devops/bureautique) : %s", title)
